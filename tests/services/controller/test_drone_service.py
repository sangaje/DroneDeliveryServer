...
def test_create_and_get_drone() -> None:
    ...
    created_drone = drone_service.create_drone(drone_id=1, **drone_data)
...
def test_get_all_drones() -> None:
    ...
    drone_service.create_drone(drone_id=1, **first_drone_data)
    drone_service.create_drone(drone_id=2, **second_drone_data)
...
def test_update_drone() -> None:
    ...
    drone_service.create_drone(drone_id=1, **drone_data)

    updated_data = {"status": DroneStatus.DELIVERING, "cur_battery": 8000.0}

    updated_drone = drone_service.update_drone(drone_id=1, drone_update=None, **updated_data)
...
def test_update_drone_with_object() -> None:
    """Test updating a drone with a drone object."""
    drone_data = {
        "drone_id": 1,
        "status": DroneStatus.IDLE,
        "max_battery": 10000.0,
    ...
    new_drone = drone_service.Drone(**new_data)
    assert new_drone is not None

    updated_drone = drone_service.update_drone(drone_id=1, drone_update=new_drone)
    assert updated_drone is not None
    assert updated_drone.status == DroneStatus.CHARGING
    assert updated_drone.cur_battery == 5000.0
